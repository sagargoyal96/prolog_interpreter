...
crux [Predicate("male",[Cons("arjun")]) ; Predicate("male",[Cons("bhima")])] [Predicate("male",[Var("X")])] []

Predicate("husband",[Var("X");Var("Y")])
Predicate("mother",[Var("X");Var("Y")])

fir_mer ([(Predicate("female",[Cons("arjun")]),[]) ; (Predicate("male",[Cons("bhima")]),[])]) (Predicate("male",[Var("x")])) [] ;;
...
brother(X,Y):- male(Y),child(X,C,A),child(Y,C,A),X\=Y.
brother(X,Y):- male(Y),child(X,A,C),child(Y,A,B),X\=Y,B\=C.
brother(X,Y):- male(Y),child(X,C,A),child(Y,B,A),X\=Y,B\=C.

sister(X,Y):- female(Y),child(X,C,A),child(Y,C,A),X\=Y.
sister(X,Y):- female(Y),child(X,A,C),child(Y,A,B),X\=Y,B\=C.
sister(X,Y):- female(Y),child(X,C,A),child(Y,B,A),X\=Y,B\=C.
